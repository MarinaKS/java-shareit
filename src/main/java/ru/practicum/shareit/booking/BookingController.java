...
public class BookingController {
    ...
    @GetMapping
    public List<BookingResponseDto> getBookingsByUserIdSorted(@RequestHeader(X_SHARER_USER_ID) long userId,
                                                              @RequestParam(value = "state", defaultValue = "ALL") @Valid State state) {
        List<Booking> bookings = bookingService.getBookingsByUserIdSorted(userId, state);
    ...
    @GetMapping("/owner")
    public List<BookingResponseDto> getBookingsByItems(@RequestHeader(X_SHARER_USER_ID) long userId,
                                                       @RequestParam(value = "state", defaultValue = "ALL") State state) {
        List<Booking> bookings = bookingService.getBookingsByItems(userId, state);
        List<BookingResponseDto> bookingDtos = new ArrayList<>();
        for (Booking booking : bookings) {
            bookingDtos.add(BookingMapper.toBookingResponseDto(booking));
        }
